...
- [ ] 診斷為什麼後面兩個 First Wave NFT 只顯示彩色背景而不是真實圖片
- [ ] 檢查 Thirdweb API 響應中是否有其他字段可以提取圖片 URL（如 metadata_url）
...
- [x] 找到 NFTDetail.tsx 中的地址顯示代碼
- [x] 移除 truncate 類 - 直接顯示完整地址
- [x] 合約地址和創建者地址完整顯示
- [x] 測試地址顯示 - 正常工作

## 🎁 結帳頁面測試贈送功能 - 已完成

- [x] 後端實現 thirdweb API 調用邏輯 (transferNFT)
- [x] 前端結帳頁面添加「測試贈送」按鈕
- [x] 實現 NFT 轉移流程（使用 safeTransferFrom）
- [x] 測試 NFT 轉移功能 - 成功執行，頁面自動跳轉回首頁

## 🖼️ NFT 圖片防盜鏈問題 - 進行中

- [ ] 後端實現圖片代理端點 (/api/proxy-image)
- [ ] 添加 Referer 頭和其他必要的請求頭
- [ ] 前端修改圖片 URL 使用代理端點
- [ ] 測試圖片顯示是否正常

## 🔄 首頁 NFT 商品刷新 - 進行中

- [ ] 修改 Home.tsx 在頁面加載時強制重新獲取 NFT 商品
- [ ] 禁用 tRPC 查詢快取，確保每次刷新都重新獲取
- [ ] 測試頁面刷新是否重新加載最新 NFT 商品
